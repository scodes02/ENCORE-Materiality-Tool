...
`Materiality Script.qmd` creates materiality heat maps and workbooks for organisations by combining their procurement data with ENCORE (Exploring Natural Capital Opportunities, Risks and Exposure) data. 
...
- Creating visual heat maps that highlight areas of highest nature pressure and dependency with tghe ability to only show the most relevant information to create concise visuals 
- Creating an Excel workbook that highlight the material pressures and dependencies of specific suppliers providing data for enhanced procurement engagement

Data Requirements

Input Files
1. Procurement Data:
   - Contains organizational spending data with taxonomy classifications in AUPN or relevant classification system. As a default, `procurement/dummy_procurement_data.csv` is provided. 

2. Classification Mappings:
   - `AUPN_into_ISIC_Classification.xlsx`: Maps organizational procurement categories to ISIC codes
   -  If not in AUPN, or a variation different to the AUPN system the University of Melbourne is used, a new mapping file with a relvant crosswalk table will be needed 

3. ENCORE Data:
   - `07. Pressure mat ratings.csv` -> tidied into `encore_pressure_ratings.csv`: Environmental pressure materiality ratings by ISIC code
   - `06. Dependency mat ratings.csv` -> tidied into `encore_pressure_ratings.csv`: Ecosystem service dependency ratings by ISIC code
   - `14. EXIOBASE NACE ISIC crosswalk.csv`: can be useful if an organisation have existing NACE or EXIOBASE classifications of spend as opposed to AUPN
